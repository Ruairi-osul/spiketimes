import pytest
from spiketimes.alignment import _align_to, _negative_align
...
class TestAlignTo:
    def test_two_scaler_input(self):
        ...
        with pytest.raises(TypeError):
            _align_to(to_be_aligned=bad_input_1, to_align_to=bad_input_2)
    ...
    def test_list_input(self):
        ...
        # one bad input
        with pytest.raises(TypeError):
            _align_to(bad_input_1, good_input)
            _align_to(good_input, bad_input_2)

        # one two bad inputs
        with pytest.raises(TypeError):
            _align_to(bad_input_1, bad_input_2)
    ...
    def test_1d_array(self):
        ...
        with pytest.raises(ValueError):
            _align_to(bad_input, good_input)
            _align_to(good_input, bad_input)
    ...
    def test_no_beyond(self):
        ...
        actual = _align_to(a, b, no_beyond=True)
        np.testing.assert_allclose(actual, expected)
...
class TestNegativeAlign:
    def test_two_scaler_input(self):
        ...
        bad_input_2 = 2
        with pytest.raises(TypeError):
            _negative_align(to_be_aligned=bad_input_1, to_align_to=bad_input_2)
    ...
    def test_list_input(self):
        ...
        # one bad input
        with pytest.raises(TypeError):
            _negative_align(bad_input_1, good_input)
            _negative_align(good_input, bad_input_2)

        # one two bad inputs
        with pytest.raises(TypeError):
            _negative_align(bad_input_1, bad_input_2)
    ...
    def test_1d_array(self):
        ...
        with pytest.raises(ValueError):
            _negative_align(bad_input, good_input)
            _negative_align(good_input, bad_input)

    def test_events_after_first_aligning_event(self):
        a = np.arange(7, 10, 1)
        b = np.arange(3.5, 10.5, 1)
        good_output = np.array([-0.5, -0.5, np.nan])
        actual_output = _negative_align(a, b)
        np.testing.assert_allclose(actual_output, good_output)
    ...
    def test_no_beyond(self):
        ...
        actual = _negative_align(a, b, no_before=True)
        np.testing.assert_allclose(actual, expected)
